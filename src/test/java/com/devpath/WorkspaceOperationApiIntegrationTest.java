...
class WorkspaceOperationApiIntegrationTest {
    ...
    void setUp() {
        ...
        integrationRepository.save(ExternalIntegration.builder()
    ...
    void groupedSwaggerIncludesWorkspaceOperationEndpoints() throws Exception {
        mockMvc.perform(get("/v3/api-docs/learner"))
    ...
    void workspaceNoticeFlowUsesJwtPrincipalAndDoesNotRequireWorkspaceIdOnRead() throws Exception {
        ...
        mockMvc.perform(get("/api/workspaces/{workspaceId}/notices", WORKSPACE_ID)
                        .with(authentication(learnerAuthentication(LEARNER_ID))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.data.length()").value(1));

        mockMvc.perform(get("/api/workspace-notices/{noticeId}", noticeId)
                        .with(authentication(learnerAuthentication(LEARNER_ID))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(noticeId))
                .andExpect(jsonPath("$.data.workspaceId").value(WORKSPACE_ID));

        mockMvc.perform(patch("/api/workspace-notices/{noticeId}", noticeId)
                        .with(authentication(learnerAuthentication(LEARNER_ID)))
        ...
        mockMvc.perform(get("/api/workspaces/{workspaceId}/notices/unread/count", WORKSPACE_ID)
                        .with(authentication(learnerAuthentication(LEARNER_ID))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").value(1));

        mockMvc.perform(post("/api/workspace-notices/{noticeId}/read", noticeId)
                        .with(authentication(learnerAuthentication(LEARNER_ID))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("SUCCESS"));
        ...
        mockMvc.perform(delete("/api/workspace-notices/{noticeId}", noticeId)
                        .with(authentication(learnerAuthentication(LEARNER_ID))))
                .andExpect(status().isOk());
    ...
    void integrationAndAdminOperationApisRemainSwaggerTestableWithAuthentication() throws Exception {
        mockMvc.perform(get("/api/workspaces/{workspaceId}/integrations", WORKSPACE_ID)
                        .with(authentication(learnerAuthentication(LEARNER_ID))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.data.length()").value(1))
        ...
        mockMvc.perform(patch("/api/workspaces/{workspaceId}/integrations/{provider}", WORKSPACE_ID, "GITHUB")
                        .with(authentication(learnerAuthentication(LEARNER_ID)))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(get("/api/admin/recommendation-settings")
                        .with(authentication(adminAuthentication())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.length()").value(greaterThanOrEqualTo(1)))
                .andExpect(jsonPath("$.data[*].settingKey").value(hasItem("algorithm.weight.recent_activity")));

        mockMvc.perform(patch("/api/admin/recommendation-settings")
                        .with(authentication(adminAuthentication()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json(Map.of(
                                "settings",
        ...
        mockMvc.perform(get("/api/admin/experiments/results")
                        .with(authentication(adminAuthentication())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.length()").value(1));

        mockMvc.perform(get("/api/admin/experiments/{experimentId}/results", "EXP-2026-001")
                        .with(authentication(adminAuthentication())))
                .andExpect(status().isOk())
    ...
    private long createNotice(String title, String content) throws Exception {
        MvcResult result = mockMvc.perform(post("/api/workspaces/{workspaceId}/notices", WORKSPACE_ID)
                        .with(authentication(learnerAuthentication(LEARNER_ID)))
                        .contentType(MediaType.APPLICATION_JSON)
