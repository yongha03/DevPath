...
// 학습 자동화 룰 저장소다.
public interface LearningAutomationRuleRepository
    extends JpaRepository<LearningAutomationRule, Long> {

  // 전체 룰 목록을 우선순위 기준으로 조회한다.
  List<LearningAutomationRule> findAllByOrderByPriorityDescIdDesc();

  // 특정 상태의 룰 목록을 우선순위 기준으로 조회한다.
  List<LearningAutomationRule> findAllByStatusOrderByPriorityDescIdDesc(
