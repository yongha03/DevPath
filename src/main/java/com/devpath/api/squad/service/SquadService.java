...
public class SquadService {
    ...
    public SquadResponse createSquad(Long userId, CreateSquadRequest request) {
        ...
        SquadMember leaderMember = SquadMember.builder()
                .squad(squad)
                .user(leader)
    ...
    @Transactional
    public SquadInvitationResponse inviteMember(Long squadId, Long inviterId, InviteSquadMemberRequest request) {
        ...
        if (squadInvitationRepository.existsBySquadAndInviteeIdAndStatus(squad, invitee.getId(), SquadInvitationStatus.PENDING)) {
            throw new CustomException(ErrorCode.SQUAD_INVITATION_ALREADY_PENDING);
        }

        SquadInvitation invitation = SquadInvitation.builder()
                .squad(squad)
                .inviterId(inviterId)
    ...
    @Transactional
    public SquadResponse updateSettings(Long squadId, Long userId, UpdateSquadSettingsRequest request) {
        Squad squad = findActiveSquad(squadId);
    ...
    private Squad findActiveSquad(Long squadId) {
        return squadRepository.findByIdAndIsDeletedFalseAndIsArchivedFalse(squadId)
                .orElseThrow(() -> new CustomException(ErrorCode.SQUAD_NOT_FOUND));
    }

    private Squad findNonDeletedSquad(Long squadId) {
        return squadRepository.findByIdAndIsDeletedFalse(squadId)
                .orElseThrow(() -> new CustomException(ErrorCode.SQUAD_NOT_FOUND));
    }
