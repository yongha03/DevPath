...
public class MentoringQuestionService {
  ...
  public QnaResponse.MentoringQuestionDetail createQuestion(
      Long mentoringId, QnaRequest.QuestionCreate request) {
    ...
    MentoringQuestion question =
        MentoringQuestion.builder()
            .mentoring(mentoring)
            .writer(writer)
            .title(request.title())
    ...
    return QnaResponse.MentoringQuestionDetail.from(
        mentoringQuestionRepository.save(question), List.of());
  }
  ...
  public QnaResponse.MentoringQuestionDetail getQuestion(Long questionId) {
    ...
    List<MentoringAnswer> answers =
        mentoringAnswerRepository.findAllByQuestion_IdAndIsDeletedFalseOrderByCreatedAtAsc(
            questionId);
  ...
  public QnaResponse.AnswerDetail createAnswer(Long questionId, QnaRequest.AnswerCreate request) {
    ...
    MentoringAnswer answer =
        MentoringAnswer.builder()
            .question(question)
            .writer(writer)
            .content(request.content())
            .build();
    ...
    // 질문 작성자가 아닌 사람이 답변하면 질문 작성자에게 알림을 남긴다.
  ...
  private void createAnswerNotificationIfNeeded(MentoringQuestion question, User answerWriter) {
    ...
    LearnerNotification notification =
        LearnerNotification.builder()
            .learnerId(question.getWriter().getId())
            .type(LearnerNotificationType.SYSTEM)
            .message("멘토링 질문에 새 답변이 등록되었습니다: " + question.getTitle())
            .build();

    learnerNotificationRepository.save(notification);
  }
}
