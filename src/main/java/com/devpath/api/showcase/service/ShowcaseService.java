...
public class ShowcaseService {
    ...
    public List<ShowcaseSummaryResponse> getShowcases(ShowcaseCategory category, ShowcaseSort sort) {
        ...
        return showcases.stream()
                .map(s -> ShowcaseSummaryResponse.of(s, showcaseLikeRepository.countByShowcaseId(s.getId())))
                .toList();
    }
    ...
    @Transactional
    public ShowcaseResponse updateShowcase(Long showcaseId, Long userId, UpdateShowcaseRequest request) {
        Showcase showcase = getShowcaseEntity(showcaseId);
        validateOwner(showcase, userId);
        showcase.update(request.getTitle(), request.getDescription(), request.getThumbnailUrl(),
                request.getCategory(), request.isPublic());
        return toResponse(showcase);
    }
    ...
    @Transactional
    public List<ShowcaseLinkResponse> updateLinks(Long showcaseId, Long userId,
            UpdateShowcaseLinksRequest request) {
        Showcase showcase = getShowcaseEntity(showcaseId);
        validateOwner(showcase, userId);
        ...
        List<ShowcaseLink> links = request.getLinks().stream()
                .map(item -> ShowcaseLink.builder()
    ...
    public Showcase getShowcaseEntity(Long showcaseId) {
        return showcaseRepository.findByIdAndIsDeletedFalse(showcaseId)
    ...
    private ShowcaseResponse toResponse(Showcase showcase) {
        ...
        List<ShowcaseLinkResponse> links = showcaseLinkRepository
