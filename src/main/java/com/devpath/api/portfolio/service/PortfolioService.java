...
public class PortfolioService {
    ...
    public PortfolioResponse getMyPortfolio(Long userId) {
        Portfolio portfolio = portfolioRepository.findFirstByUserIdAndIsDeletedFalseOrderByCreatedAtDesc(userId)
    ...
    @Transactional
    public PortfolioResponse updatePortfolio(Long portfolioId, Long userId, UpdatePortfolioRequest request) {
        Portfolio portfolio = getPortfolioEntity(portfolioId);
    ...
    @Transactional
    public PortfolioItemResponse addItem(Long portfolioId, Long userId,
            PortfolioItemType itemType, AddPortfolioItemRequest request) {
        Portfolio portfolio = getPortfolioEntity(portfolioId);
        validateOwner(portfolio, userId);

        int nextOrder = portfolioItemRepository
                .findAllByPortfolioIdOrderBySortOrderAsc(portfolioId).size();
    ...
    @Transactional
    public PortfolioGithubCommitResponse addGithubCommit(Long portfolioId, Long userId,
            AddGithubCommitRequest request) {
        Portfolio portfolio = getPortfolioEntity(portfolioId);
        validateOwner(portfolio, userId);

        PortfolioGithubCommit commit = PortfolioGithubCommit.builder()
                .portfolioId(portfolioId)
    ...
    private PortfolioResponse toResponse(Portfolio portfolio) {
        List<PortfolioItemResponse> items = portfolioItemRepository
                .findAllByPortfolioIdOrderBySortOrderAsc(portfolio.getId())
                .stream().map(PortfolioItemResponse::from).toList();

        List<PortfolioGithubCommitResponse> commits = portfolioGithubCommitRepository
                .findAllByPortfolioIdOrderByCommittedAtDesc(portfolio.getId())
                .stream().map(PortfolioGithubCommitResponse::from).toList();
